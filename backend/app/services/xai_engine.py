...
def explain_dcpi(features: dict, reasons: list) -> str:
    top = reasons[:3]
    phrases = [
...
def get_or_create_explanation(db, dcpi: DCPIScore) -> XAIExplanation:
    existing = db.execute(
        select(XAIExplanation)
        .where(XAIExplanation.dcpi_id == dcpi.id)
        .order_by(XAIExplanation.id.desc())
        .limit(1)
    ).scalar_one_or_none()
    if existing:
        return existing

    nl = explain_dcpi(dcpi.features, dcpi.reasons)
    features = [[r["feature"], r["contribution"]] for r in dcpi.reasons]
    row = XAIExplanation(
        dcpi_id=dcpi.id,
        natural_language=nl,
        features=features,
